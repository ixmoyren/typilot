...
class TypstCodeBlockInjector : MultiHostInjector {
    ...
    fun findLanguage(langName: String): Language? {
        languageAliases[langName]?.let { alias ->
            Language.findLanguageByID(alias)?.let { return it }
        }
        Language.getRegisteredLanguages().firstOrNull {
            it.id.equals(langName, ignoreCase = true)
        }?.let { return it }
        Language.getRegisteredLanguages().firstOrNull {
    ...
    override fun getLanguagesToInject(
        ...
        textNodes.forEachIndexed { index, textNode ->
            val relativeRange = TextRange(
                textNode.textRange.startOffset - hostStart,
                textNode.textRange.endOffset - hostStart
            )
