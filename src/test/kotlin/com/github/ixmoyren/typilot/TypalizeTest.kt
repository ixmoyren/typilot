...
class TypalizeTest : BasePlatformTestCase() {
    fun testTypstVersion() {
        val versionResult = typalizer.version() ?: throw IllegalStateException("The typst lexer returned null")
    ...
    fun testTypstTokenize() {
        val text = ""
        val tokenResult = typalizer.tokenize(text) ?: throw IllegalStateException("The typst lexer returned null")
        if (tokenResult.failure()) {
            throw IllegalStateException("The typst lexer couldn't work.", tokenResult.error)
        }
        val tokens = tokenResult.result?.value ?: throw IllegalStateException("The typst tokenize result is null")
