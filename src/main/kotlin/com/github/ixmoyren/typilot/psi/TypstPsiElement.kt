...
sealed interface TypstPsiElement : NavigatablePsiElement {
    ...
    val kind: TypstSyntaxKind? get() = (node.elementType as? TypstElementType)?.kind

    val tag: TypstHighlightTag? get() = node.getUserData(TypstHighlightTagKeys.TAG)
}

sealed class ATypstPsiElement(node: ASTNode) : ASTWrapperPsiElement(node), TypstPsiElement {
    override fun accept(visitor: PsiElementVisitor) {
        if (visitor is TypstPsiElementVisitor) accept(visitor)
...
class TypstRawBlockPsiElement(node: ASTNode) : ATypstPsiElement(node), PsiLanguageInjectionHost {
    ...
    fun textChildren(): List<PsiElement> =
        node.children()
            .filter { child ->
                val type = child.elementType ?: return@filter false
                when (type) {
            ...
            .map { node -> node.psi!! }.toCollection(mutableListOf())
...
val TypstSyntaxKindToPsiElementMap: Map<TypstSyntaxKind, (ASTNode) -> PsiElement> = mapOf(
