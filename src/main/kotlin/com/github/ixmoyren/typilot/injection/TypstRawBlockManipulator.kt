...
class TypstRawBlockManipulator: AbstractElementManipulator<TypstRawBlockPsiElement>() {
    override fun handleContentChange(
        element: TypstRawBlockPsiElement,
        range: TextRange,
        newContent: String
    ): TypstRawBlockPsiElement {
        val newText = range.replace(element.text, newContent)
        val type = element.containingFile.fileType
        val newFile = PsiFileFactory.getInstance(element.project)
            .createFileFromText("new." + type.defaultExtension, type, newText)
