...
class TypstElementType(val kind: TypstSyntaxKind?, name: String = kind!!.name) : IElementType(name, TypstLanguage.INSTANCE) {
    ...
    companion object {
        ...
        val COMMENT_TOKEN_SET by lazy { TokenSet.create(*TypstSyntaxKind.COMMENT_SET.map { it.elementType }.toTypedArray()) }

        val WHITESPACE_TOKEN_SET by lazy { TokenSet.create(*TypstSyntaxKind.SPACE_SET.map { it.elementType }.toTypedArray()) }
